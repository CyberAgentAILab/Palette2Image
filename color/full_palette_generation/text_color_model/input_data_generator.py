...
from color.full_palette_generation.text_color_model.model_config import Config
...
class TextEmbeddings:    
    ...
    def make_text_data(self, config):
        text_input_emb = []
        text_input_emb_ = self.make_and_parse_text_emb(config['Text_Input_Emb_File_Path'])
        for tc in text_input_emb_:
            contents = tc.split(' ')
            contents = [float(c) for c in contents] # convert string to float
            text_input_emb.append(contents)
            
        return text_input_emb
...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, config):
        ...
        self.text_input_emb= self.text_embeddings.make_text_data(config)
    ...
    def make_segment_inputs(self, batch_token_id):
        ...
        for i in range(len(batch_token_id)):
            # fixed segmentation
            one_segment_inputs = [0, 0, 0, 0, 0, 0, 0]
    ...
    def make_text_embeddings(self, idx):
        batch_text_input_emb = []
        # get text contents emb and image labels emb separately with max_*_length
        for i in range(self.batch_size):
            for j in range(self.config['Max_Text_Input_Length']):
                batch_text_input_emb.append(self.text_input_emb[idx * self.batch_size + i * self.config['Max_Text_Input_Length'] + j])
        batch_text_input_emb = np.array(batch_text_input_emb).reshape(self.batch_size, self.config['Max_Text_Input_Length'], -1)
        return batch_text_input_emb
    ...
    def __getitem__(self, idx):
        ...
        batch_text_input_emb = self.make_text_embeddings(idx)
        ...
        batch_text_input_emb = np.array(batch_text_input_emb)[shuffle]
        
        return batch_x, batch_mlm_mask, batch_mcc_mask, origin_x, batch_segment, batch_padding_mask, batch_text_input_emb
...
if __name__ == '__main__':
    ...
    batch_x,  batch_mlm_mask, batch_mcc_mask, origin_x, batch_segment, batch_padding_mask, batch_text_input_emb = dataset[0]
    ...
    print(f'text_input_emb: {batch_text_input_emb.shape}')